...
import org.sonar.wsclient.connectors.util.HttpClientTrustingAllCerts;
 ...
 * @since 2.1
 */
public class HttpsClient4Connector extends Connector {

  private Host server;
  private DefaultHttpClient client;

  public HttpsClient4Connector(Host server) {
    this.server = server;
    initClient();
  }
  ...
  private void initClient() {
    //client = new DefaultHttpClient();
		HttpClientTrustingAllCerts httpClientTrustingAllCerts = new HttpClientTrustingAllCerts();
		try {
			client = httpClientTrustingAllCerts
					.getHttpClientTrustingAllSSLCerts();

			HttpParams params = client.getParams();
			HttpConnectionParams.setConnectionTimeout(params,
					AbstractQuery.DEFAULT_TIMEOUT_MILLISECONDS);
